...
parser.add_argument('--batch_size', type=int, default=1, metavar='N', help='batch size for training(default: 128)')
...
parser.add_argument('--input_dim', type=int, default=256 * 256, metavar='N', help='input dim(default: 28*28 for MNIST)')
...
def test(model, optimizer, test_dataset_loader, epoch, best_test_loss):
    test_avg_loss = 0.0
    with torch.no_grad():  # 这一部分不计算梯度，也就是不放入计算图中去
        '''测试测试集中的数据'''
        # 计算所有batch的损失函数的和
        for test_batch_index, (test_x, _) in enumerate(test_dataset_loader):
        ...
        # 对和求平均，得到每一张图片的平均损失
        test_avg_loss /= len(test_dataset_loader.dataset)

        '''测试随机生成的隐变量'''
        # 随机从隐变量的分布中取隐变量
        z = torch.randn(args.batch_size, args.z_dim).to(device)  # 每一行是一个隐变量，总共有batch_size行
        # 对隐变量重构
        random_res = model.decode(z).view(-1, 1, 256, 256)
        # 保存重构结果
...
def main():
    ...
    train_dataset = DataLoader(dataset=train_dataset, batch_size=1)
    test_dataset = DataLoader(dataset=test_dataset, batch_size=1)
    # 查看每一个batch图片的规模
    x, label = iter(train_dataset).__next__()  # 取出第一批(batch)训练所用的数据集
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_index, (x, _) in enumerate(train_dataset):
            ...
            if batch_index == 0:
                ...
                x_concat = torch.cat([x.view(-1, 1, 256, 256), x_hat.view(-1, 1, 256, 256)], dim=3)
        ...
        if (epoch + 1) % args.test_every == 0:
            best_test_loss = test(model, optimizer, test_dataset, epoch, best_test_loss)
    return loss_epoch
